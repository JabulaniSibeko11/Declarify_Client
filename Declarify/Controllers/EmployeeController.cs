...
namespace Declarify.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        public EmployeeController(
          ...
          ILogger<EmployeeController> logger, UserManager<ApplicationUser> userManager,   CentralHubApiService centralHub,ApplicationDbContext db, IReviewHelperService reviewHelper, IDoiPdfService pdf)
        {
        ...
        [AllowAnonymous]
        [HttpGet("task")]
        public async Task<IActionResult> Task([FromQuery] string token, CancellationToken ct)
        {
            try
            {
                ...
                // ✅ Credit check only (block if balance < 1)
                var creditCheck = await _centralHub.CheckCreditBalance();
                if (creditCheck == null)
                {
                    TempData["ErrorCheckCredits"] = "Cannot verify credits — license server unreachable. Please try again later.";
                    return View("NotEnoughCredits");
                }
                if (!creditCheck.hasCredits || creditCheck.currentBalance < 1)
                {
                    TempData["ErrorCheckCredits"] = "Your organization has no remaining credits. Please contact your administrator to top up.";
                    return View("NotEnoughCredits");
                }
                ...
                // ✅ latest submission (used for Amendment/resubmission + prefill)
                ...
                var employeeSignature = employee?.Signature_Picture ?? string.Empty;
                var employeeName = employee?.Full_Name ?? string.Empty;
                var employeeEmail = employee?.Email_Address ?? string.Empty;

                // ✅ Can submit if Outstanding OR AmendmentRequired
                var canSubmit = task.Status == "Outstanding" || task.Status == "AmendmentRequired";
                ...
                var viewModel = new TaskDetailViewModel
                {
                    ...
                    // ✅ this makes the form prefill like your “draftData” logic
                    ...
                    // ✅ if your VM supports these fields
                    AmendmentRequired = task.Status == "AmendmentRequired",
                    AmendmentReason = task.AmendmentReason
                };
            ...
            {
                _logger.LogError(ex, "Error loading task with token {Token}", token);
        ...
        public async Task<IActionResult> Submit([FromBody] SubmitFormRequest request)
        {
            ...
            if (creditCheck == null)
            {
                ...
                {
                    success = false,
                    message = "Cannot verify credits — license server unreachable. Please try again later."
                });
            }
            ...
            if (!creditCheck.hasCredits || creditCheck.currentBalance < 1)
            {
                ...
                {
                    success = false,
                    message = "Your organization has no remaining credits. Please contact your administrator to top up."
                });
            }


            //2. Deduct credits via central hub API
            var consumeResult = await _centralHub.ConsumeCredits(
     1,
     $"DOI Final Submission - Token {request.Token}"
 );
        ...
        public async Task<IActionResult> Review(int? taskId, int? submissionId)
        {
            try
            {
                ...
                var viewModel = new ReviewSubmissionViewModel
                {
                    ...
                    TaskId = submission.FormTaskId,
                    TaskAccessToken = submission.Task.AccessToken
                };
        ...
        [HttpGet("download-doi-pdf")]
        public async Task<IActionResult> DownloadDoiPdf(string token, CancellationToken ct)
        {
            if (string.IsNullOrWhiteSpace(token))
            ...
            var submission = await _db.DOIFormSubmissions
                .Where(s => s.FormTaskId == task.TaskId &&
                       (s.Status == "Submitted" || s.Status == "Reviewed" || s.Status == "Approved"))
                .OrderByDescending(s => s.VersionNo)
                .FirstOrDefaultAsync(ct);

            if (submission is null)
                return NotFound("No submitted DOI found.");

            var path = submission.PdfFilePath;
            var fileName = submission.PdfFileName;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> RequestAmendment(int taskId, string reason, CancellationToken ct)
        {
            ...
            task.Status = "AmendmentRequired";
            task.IsAmendmentRequired = true;
            task.AmendmentReason = reason.Trim();
            task.AmendmentRequestedAt = DateTime.UtcNow;
            task.AmendmentRequestedBy = User.Identity?.Name ?? "Reviewer";

            await _db.SaveChangesAsync(ct);

            // OPTIONAL: email employee here (if your email is already wired)
            // await _email.SendAmendmentRequestAsync(task.Employee.Email, task.AccessToken, reason);

            TempData["Success"] = "Amendment request sent to employee.";
            return RedirectToAction("Review", new { taskId });
        }
